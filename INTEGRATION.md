...
| 04 聯絡 | `/contact/`：30 分鐘對話、三步驟 | `/contact/` |
| 頁尾 | 公司名稱、地址、Email、營業時間、統編 | 全站 |
| `engine.html`（獨立頁） | `/engine/`：5 × 3 矩陣、CPA/ROAS 數字、六項產品能力、六步流程、FAQ（不放價格，價格留在 `/engine/`） | 取代 `/engine/`，或作為它的新版 |
...
不必每一頁都放 3D，內頁維持輕量，但共用同一套設計語言就會有整體感。`engine.html` 就是照這個原則做的第一頁範本，其他內頁可以直接複製它的骨架：
...
- **元件**：`secret-pathways-assets/inner.css` 已抽出內頁會用到的 token、導覽列、掃描線、能力格、流程、FAQ、CTA 與頁尾；首頁裡的 `.crow`（案例列）、`.les`（服務圖版）、`.steps`（步驟）也可以照樣搬過去。
...
- 保留 Yoast 的 Organization schema；可以再加 `Service` 結構化資料（方案價格留在 `/engine/` 子頁的 `Offer`）。
...
3. 內容數字（客戶營業額、案例成果）集中到一處管理，首頁與內頁共用；案例照片換成客戶授權的實景照。
4. 之後若要做更多頁面的 3D 場景，可以複用同一座看台的不同鏡頭（`CAM` 陣列加一個航點即可）。
