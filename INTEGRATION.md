...
| 04 AI 素材引擎 | `/engine/`：5 × 3 矩陣、CPA/ROAS 數字、六項產品能力、六步流程（首頁不放價格，價格留在 `/engine/`） | `/engine/`、`/contact/` |
...
- **元件**：首頁裡的 `.crow`（案例列）、`.les`（服務圖版）、`.caps`（能力格）、`.proc`（流程）、`.steps`（步驟）都是純 CSS，可直接搬到內頁重用。
