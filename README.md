...
- **即時 3D 場景**：CIT 看台——弧形階梯看台（每四排一道 LED 燈條）、懸挑屋頂與桁架、屋頂上蓋著琉璃瓦頂的記者樓、看台後方的玻璃辦公空間（每扇窗都是程式生成的貼圖）、白色欄杆、兩座照明燈塔、後方的捷運高架與定時通過的列車、一排高樓、遠處的台北 101（錐形基座、八節竹節、尖塔、燈帶與銅錢徽飾）與圓山大飯店（紅柱、大屋頂、暖光）、入口門架（LED 指示牌）、燈柱、珊瑚色月亮、有燈火的天際線、雨、霧、數據光點、航空警示燈。
...
- **中／EN 雙語**：英文版是獨立的頁面（`en/index.html`、`en/engine.html`），由字典 `secret-pathways-assets/i18n.js` 在建置時自動產生（`node build-en.mjs`），中英互加 `hreflang`、各自有 canonical，兩種語言都能被搜尋引擎與 AI 搜尋獨立收錄。導覽列右側的「中 / EN」連到對應頁面；英文標題用大寫無襯線排版，3D 場景裡的招牌維持中文。改文案的流程：改中文 HTML 與字典，再跑一次建置。之後每一頁都照這套做，新增頁面的步驟見 `INTEGRATION.md`。
